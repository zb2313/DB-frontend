<template>
  ...
  <el-dropdown-menu slot="dropdown" >
    ...
    <el-dropdown-item command="2021">2021</el-dropdown-item>
        ...
        <el-carousel 
        indicator-position="outside" :autoplay=false height="300px">
    <el-carousel-item >
        <div class="css1">
在这一年里，你一共在旅道消费了{{flight_order_num+train_order_num+attraction_order_num}}笔订单
    ...
    <el-carousel-item >
      旅行距离共计{{journey_length}}公里 
        相当于跑了{{journey_length/42.195}}个全程马拉松
    </el-carousel-item>
   ...
   async mounted(){
    ...
    methods: {
      async get_data(){
        ...
        .then((response)=>{this.origin_plans=response;
        console.log(1);
        })
       await this.$axios.get("http://49.234.18.247:8080/api/FunGetAttractionInfoByUserId/"+localStorage.getItem("ms_username"))
        .then((response)=>{
          this.origin_attractions=response;console.log(2);
        })
       await this.$axios
        .get("http://49.234.18.247:8080/api/FunGetTrainTicketByUid/"+localStorage.getItem("ms_username"))
        .then((response)=>{
          this.origin_train=response;console.log(3);
        })
      await this.$axios
        .get("http://49.234.18.247:8080/api/FunGetFlightTicketByUid/"+localStorage.getItem("ms_username"))
        .then((response)=>{
          this.origin_flight=response;console.log(4);
        })
        console.log('get!')
     ...
     this.flight=this.origin_flight.data.filter(function (value) {
      ...
      async compute_data(){
        ...
        //plan
            ...
            //this.plans=this.origin_plans.data;
           // console.log(this.plans)
            let temp=[];
            this.like_num=0;
    ...
    //trainticket
     //this.trainticket=this.origin_train.data;
      ...
      this.train_order_num=this.trainticket.length;
          ...
          for(let i=0;i<this.trainticket.length;i++){
            this.journey_length+=await this.compute_distance(this.trainticket[i].starT_LOCATION,this.trainticket[i].enD_LOCATION);
          ...
          for(let x of s)
          this.unique_cities.add(x);
        //flight
        this.flight_order_num=this.flight.length;
          var cities2=[];
          var s2=new Set();
          for(let i=0;i<this.flight.length;i++){
            this.journey_length+=await this.compute_distance(this.flight[i].starT_LOCATION,this.flight[i].enD_LOCATION);
     ...
     async compute_distance(s,t){
       var l1,l2;
     await  fetch("https://restapi.amap.com/v3/geocode/geo?key=f7171076bbd21882cf1c0a5ae7be2725&address="+s)
     ...
     await  fetch("https://restapi.amap.com/v3/geocode/geo?key=f7171076bbd21882cf1c0a5ae7be2725&address="+t)
                .then(function (response) {
                return response.json();
                })
                .then((response)=>{
